...
class DataBaseHandler:
    ...
    def get_most_expensive_track_names(self, number_of_tracks: int) -> tp.Sequence[tuple[str]]:
        ...
        :return:
        """
        self.cursor.execute("SELECT Name FROM tracks ORDER BY UnitPrice DESC, TrackId ASC LIMIT ?", (number_of_tracks,))
    ...
    def get_tracks_of_given_genres(self, genres: tp.Sequence[str], number_of_tracks: int) -> tp.Sequence[tuple[str]]:
        ...
        self.cursor.execute("""
                ...
                LIMIT ?
                """.format(placeholders=','.join('?' for _ in genres)), (*genres, number_of_tracks))
        return self.cursor.fetchall()
    ...
    def get_tracks_that_belong_to_playlist_found_by_name(self, name_needle: str) -> tp.Sequence[tuple[str, str]]:
        ...
        """
        self.cursor.execute(
            "SELECT Tracks.Name, Playlists.Name FROM Playlists JOIN Playlist_Track ON Playlists.PlaylistId = "
            "Playlist_Track.PlaylistId JOIN Tracks ON Playlist_Track.TrackId = Tracks.TrackId WHERE Playlists.Name "
            "LIKE ?",
            (f"%{name_needle}%",))
        return self.cursor.fetchall()
